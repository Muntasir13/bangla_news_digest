...
class LocalDriverBaseClass(ABC):
    def __init__(self, config: dict[str, bool | str]) -> None:
        ...
        Args:
            config (dict[str, bool | sstr]): the web driver config
        """
        self.driver_config = config
        self.driver = self._init_driver()
        self.enable_network_filtering()
    ...
    def enable_network_filtering(self) -> None:
...
class ChromeLocalDriver(LocalDriverBaseClass):
    ...
    def _init_driver(self) -> WebDriver | uc.Chrome:
        ...
        for config in self.driver_config:
            if isinstance(config, bool):
                options.add_argument(f"--{config}")
            elif isinstance(config, str):
                options.add_argument(f"--{config}={self.driver_config[config]}")
...
def load_webdriver(driver_type: str, driver_config: dict[str, bool | str]) -> WebDriver:
    """Returns a local driver based on the config set up in default.yaml

    Args:
        driver_type (str): the type of driver to load
        driver_config (dict[str, str]): the driver config for the web driver
