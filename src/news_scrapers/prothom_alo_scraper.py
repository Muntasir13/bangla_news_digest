...
class ProthomAloScraper(BaseScraper):
    ...
    def extract_publishing_datetime(self) -> datetime:
        try:
            ...
            date_and_time = ": ".join(publishing_datetime_list[0].text.split(": ")[1:])
            # Final Format: 08 October 2025, 01: 37
            return datetime.strptime(bangla_to_english_datetime_parsing(date_and_time), "%d %B %Y, %H: %M")
